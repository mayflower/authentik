...
func TestLDAPBind_NegativeCachePersistsInvalidCredentials(t *testing.T) {
	...
	c1 := h.NewConn(t)
	rc, err := srv.Bind(ldap.BindRequest{BindDN: bindDN, Password: bindPW}, c1)
	if err != nil {
	...
	c2 := h.NewConn(t)
	rc2, err := srv.Bind(ldap.BindRequest{BindDN: bindDN, Password: bindPW}, c2)
	if err != nil {
	...
	h.Scenario.BackendUp = false
	c3 := h.NewConn(t)
	rc3, err := srv.Bind(ldap.BindRequest{BindDN: bindDN, Password: bindPW}, c3)
	if err != nil {
